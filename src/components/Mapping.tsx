...
import { makeCarIcon, makeMarkerIcon, Map } from "../lib/map";
...
interface Route {
  id: number;
  title: string;
  startPosition: ReadonlyLatLngLiteral;
  endPosition: ReadonlyLatLngLiteral;
}
...
export const Mapping: FunctionComponent = () => {
  ...
  const [routeIdSelected, setRouteIdSelected] = useState<number | string>("");
  ...
  useEffect(() => {
    ...
    const handler = (data: {
      ...
      const route = routes.find(
        (route) => route.id === Number(data.routeId)
      ) as Route;
  ...
  const startRoute = useCallback(
    (event: FormEvent) => {
      ...
      const route = routes.find(
        (route) => route.id === Number(routeIdSelected)
      );
